...
p <- ggplot(dat_homelessness, aes(x = date, y = n, group = category)) +
  
  # Map color to the capitalized category for the tooltip
  geom_line(aes(color = category), 
                linewidth = 1) +
  
  geom_point(aes(color = category, text = paste(
    "Date:", format(date, "%Y-%m-%d"),
    "<br>Count:", n,
    "<br>Category:", str_to_title(category) # This is the key change
  )), size = 1) +
  ...
  # Use manual colors to specify the correct hex codes
  scale_color_manual(name = "", 
                     values = c("Unsheltered" = "#8C183E", "Sheltered" = "#72CCD4")) +
...
# Style the traces to remove unwanted tooltips
p_plotly <- p_plotly %>%
  ...
  plotly::style(hoverinfo = "none", traces = c(1, 2)) %>% # Traces 1 and 3 are the lines
