...
p <- ggplot(dat_homelessness, aes(x = date, y = n)) +
  ...
  geom_point(data = filter(dat_homelessness, category == "Sheltered"), 
             size = 1, color = "#72CCD4", alpha = 0.5,
             aes(text = paste("Date:", date, "<br>Count:", n))) +
...
# Convert the ggplot object to a plotly object with the correct tooltip
p_plotly <- ggplotly(p, tooltip = "text")

# Add the static text labels as annotations
p_plotly <- p_plotly %>%
  add_annotations(
    x = as.Date("2011-01-01"), y = 1900,
    text = "Unsheltered",
    showarrow = FALSE,
    font = list(color = "#8C183E")
  ) %>%
  add_annotations(
    x = as.Date("2011-01-01"), y = 2900,
    text = "Sheltered",
    showarrow = FALSE,
    font = list(color = "#72CCD4")
  )
